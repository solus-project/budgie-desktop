...
static void budgie_applet_info_get_property(GObject *object, guint id,
...
static void budgie_applet_info_set_property(GObject *object, guint id,
...
static void budgie_applet_info_class_init(BudgieAppletInfoClass *klazz)
{
        ...
        obj_properties[PROP_ICON] = g_param_spec_string("icon",
                "Applet icon", "Set the applet icon",
                NULL, G_PARAM_READWRITE);

        obj_properties[PROP_NAME] = g_param_spec_string("name",
                "Applet name", "Set the applet name",
                NULL, G_PARAM_READWRITE);

        obj_properties[PROP_DESCRIPTION] = g_param_spec_string("description",
                "Applet description", "Set the applet description",
                NULL, G_PARAM_READWRITE);

        obj_properties[PROP_UUID] = g_param_spec_string("uuid",
                "Applet UUID", "Set the applet UUID",
                NULL, G_PARAM_READWRITE);

        obj_properties[PROP_ALIGNMENT] = g_param_spec_string("alignment",
                "Applet alignment", "Set the applet alignment",
                "start", G_PARAM_READWRITE);

        obj_properties[PROP_POSITION] = g_param_spec_int("position",
                "Applet position", "Set the applet position",
                -1000, 1000, 0, G_PARAM_READWRITE);

        /**
         * BudgieAppletInfo:settings: (type GSettings)
         */
        obj_properties[PROP_SETTINGS] = g_param_spec_pointer("settings",
                "Applet Settings", "Set the applet GSettings", G_PARAM_READWRITE|G_PARAM_CONSTRUCT);

        /**
         * BudgieAppletInfo:applet: (type BudgieApplet)
         */
        obj_properties[PROP_APPLET] = g_param_spec_pointer("applet",
                "Applet instance", "Set the applet instance", G_PARAM_READWRITE|G_PARAM_CONSTRUCT);
...
BudgieAppletInfo *budgie_applet_info_new(PeasPluginInfo *plugin_info, const char *uuid, BudgieApplet *applet, GSettings *settings)
{
        if (plugin_info) {
                return g_object_new(BUDGIE_TYPE_APPLET_INFO,
                        ...
                        "uuid", uuid, "applet", applet, "settings", settings, NULL);
        } else {
                return g_object_new(BUDGIE_TYPE_APPLET_INFO,
                        "uuid", uuid, "applet", applet, "settings", settings, NULL);
        }
